  ...
  return (
    <HydrationGate>
      <AppShell active="progress">
        ...
        <section className="progress-modules"><div className="section-heading"><div><span className="eyebrow">Module record</span><h2>Learning pathway</h2></div><span className="section-meta">{isPresentationDemo ? "Demo access enabled" : "Scores never decrease"}</span></div>{learningModules.map((module) => { const isFirst = module.position === 1; const unlocked = isPresentationDemo || isFirst || (module.position === 2 && passed); return <article key={module.id}><span className={`progress-module-icon ${unlocked ? "available" : "locked"}`}>{unlocked ? <Icon name={isFirst && passed ? "check" : "book"} /> : <Icon name="lock" />}</span><div className="progress-module-copy"><span>Module {module.position}</span><h3>{module.title}</h3><p>{module.description}</p>{isFirst ? <div className="linear-progress"><span style={{ width: `${progress}%` }} /></div> : null}</div><div className="progress-module-status"><strong>{isFirst ? `${progress}%` : unlocked ? "Ready" : "Locked"}</strong><span>{isFirst ? passed ? `Passed · ${bestScore}%` : `${state.completedLessonSlugs.length}/${moduleOneLessons.length} lessons` : isPresentationDemo ? "Unlocked for demo" : unlocked ? "Newly unlocked" : `Complete Module ${module.position - 1}`}</span>{isFirst ? <Link className="text-link" href="/learn/module-1">Open module <Icon name="arrow-right" /></Link> : module.position === 2 && unlocked ? <Link className="text-link" href="/learn/module-2">Open module <Icon name="arrow-right" /></Link> : null}</div></article>; })}</section>
