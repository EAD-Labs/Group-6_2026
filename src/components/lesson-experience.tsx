      ...
      <AppShell active="learn">
        ...
        <div className="lesson-layout">
          <article className="lesson-main">
            ...
            <section className="lesson-resources" aria-labelledby="lesson-resources-title"><div className="section-heading compact"><div><span className="eyebrow">Watch and read</span><h2 id="lesson-resources-title">Curated resources</h2></div><span>{lesson.resources.length} selected</span></div><ul>{lesson.resources.map((resource) => <li key={resource.url}><span><Icon name={resource.type === "video" ? "play" : "document"} /></span><div><small>{resource.type === "video" ? "Video" : "Guide"}</small><a href={resource.url} rel="noreferrer" target="_blank">{resource.label}</a></div><Icon name="arrow-right" /></li>)}</ul></section>
